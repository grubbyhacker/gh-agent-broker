...
func TestAuthorityRESTSessionReassignmentContract(t *testing.T) {
	...
	bad := []byte(`{"session_binding":"rest-session","session_lineage_id":"` + lease.SessionLineageID + `","predecessor_worker_id":"rest-old","predecessor_worker_fence_epoch":1,"idempotency_key":"rest-reassign-2","image":"forbidden"}`)
